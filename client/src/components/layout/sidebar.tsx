...
export default function Sidebar({ activeSection, onSectionChange, darkMode, onToggleDark }: SidebarProps) {
  ...
  return (
    <div className={`flex flex-col transition-all duration-300 flex-shrink-0 ${collapsed ? "w-16" : "w-60"}`}
      style={{ background: "hsl(var(--sidebar))", borderRight: "1px solid hsl(var(--sidebar-border))" }}>
      {/* Logo */}
      <div className="flex items-center justify-between px-4 py-4 border-b flex-shrink-0" style={{ borderColor: "hsl(var(--sidebar-border))" }}>
        {!collapsed && (
      ...
      <nav className="flex-1 px-2 py-3 overflow-y-auto space-y-4">
        {menuGroups.map(group => (
          <div key={group.label}>
            {!collapsed && (
              <div className="px-3 pb-1 text-[10px] font-semibold uppercase tracking-widest" style={{ color: "hsl(var(--sidebar-foreground) / 0.35)" }}>
                {group.label}
              </div>
            )}
                  ...
                  <button key={item.id} data-testid={`nav-${item.id}`}
                    ...
                    {!collapsed && (
                      <div className="flex-1 text-left min-w-0">
                        ...
                        <div className="text-[10px] opacity-40 truncate">{item.labelEn}</div>
                      </div>
                    )}
                  </button>
                );
      ...
      {/* Footer */}
      <div className="px-2 py-3 border-t space-y-1.5 flex-shrink-0" style={{ borderColor: "hsl(var(--sidebar-border))" }}>
        <button onClick={onToggleDark} className={`sidebar-nav-item w-full ${collapsed ? "justify-center px-2" : ""}`}
        ...
        {!collapsed && (
          <div className="flex items-center gap-2 px-3 py-2 rounded-lg" style={{ background: "hsl(var(--sidebar-accent))" }}>
            ...
            <div className="min-w-0">
              ...
              <div className="text-[10px] truncate" style={{ color: "hsl(var(--sidebar-foreground) / 0.5)" }}>admin@school.edu</div>
            </div>
          </div>
        )}
      </div>
